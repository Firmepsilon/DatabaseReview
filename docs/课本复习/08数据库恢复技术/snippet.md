...
- 事务故障的恢复由系统自动完成，**不需要用户干预**
  ...
  - (2) 对该事务的更新操作进行Undo处理。即将日志记录中“更新前的值”（Before Image, BI）写入数据库。
  - (3) 继续反向扫描日志文件，查找该事务的其他更新操作，并做同样处理。
  - (4) 如此处理下去，直至读到此事务的开始标记，事务故障恢复就完成了。
  
   > 为什么反向扫描？因为同一个数据项可能被同一事务多次修改，只有逆序 Undo 才能一步步恢复到最初的值。正向 Undo 会导致恢复错误。
   > 
   > 例如：事务①X:10->20，事务②X：20->30。正向扫描会恢复为20，反向扫描恢复为10。
...
- (1) 装入最新的后备数据库副本，使数据库恢复到最近一次转储时的一致性状态。
  ...
  - 对于==动态转储==的数据库副本，还须同时装入转储时刻的日志文件副本,利用与**恢复系统故障相同的方法**(即REDO+UNDO),才能将数据库恢复到一致性状态
...
### 检查点记录的内容
  1. 建立检查点时刻所有**正在执行的事务**清单
  2. 各个事务最近一个日志记录的地址

---
**例子：**
假设在时刻 Tc（建立检查点）时，有两个正在执行的事务：T1 和 T2，它们的日志记录如下：
```
<Start T1>          -- 地址: LSN 100
<T1, A, 100, 50>    -- 地址: LSN 101  (T1 修改了A)
<Start T2>          -- 地址: LSN 102
<T2, B, 200, 150>   -- 地址: LSN 103  (T2 修改了B)
<T1, C, 300, 350>   -- 地址: LSN 104  (T1 又修改了C)
← 此刻建立检查点 Tc →
```
在这个检查点记录里，会保存：
1. 活跃事务清单：{T1, T2}
2. 各事务最近一个记录的地址：
   - 对 T1：最后一条日志是 <T1, C, 300, 350> → 地址 LSN 104
   - 对 T2：最后一条日志是 <T2, B, 200, 150> → 地址 LSN 103
--- 
...
- 建立检查点原则
  - 定期：按照预定的一个时间间隔
  - 不定期：按照某种规则，如日志文件已写满一半建立一个检查

### 利用检查点的恢复步骤
1.  从“重启动文件” 读出最近检查点的地址，并定出检查点在日志文件中的位置。
2.  创建Redo表(空)，Undo表(即**检查点对应的活动事务表**)。
3.  前向检索日志文件，如果遇到 Start 记录（这个一般是检查点后的新事务），则将对应的事务加入 Undo 表；如果遇到 Commit 记录，则将该记录对应的事务从 Undo 表移到 Redo 表。
4.  反向检索日志文件，对 Undo 表中的事务，按照日志记录，做Undo操作，直到遇到对应的 Start 记录。
5.  正向检索 Redo 事务的日志记录，做 Redo 操作，直到对应的 Commit 记录。
---
...
> 频繁地复制数据自然会降低系统运行效率；
> 
> 在实际应用中往往只选择对**关键数据和日志文件**进行镜像，而不是对整个数据库进行镜像。
